...
-- ============================================================
-- ANTI KICK (portado do DUEL UNDETECTED)
-- Deteccao de carry por atributos/valores + modelos e tools,
-- e bloqueio total de velocidade extra enquanto carregando.
-- ============================================================
ASTA.carryFlags = {"Carrying","IsCarrying","Grabbed","Holding","StealHold","HasGrab"}

function ASTA.akIsCarryName(name)
    local n = tostring(name or ""):lower()
    return n:find("brainrot") or n:find("animal") or n:find("carry") or n:find("grab")
        or n:find("steal") or n:find("hold") or n:find("skibidi") or n:find("toilet")
end

function ASTA.akIsIgnoredTool(name)
    local n = tostring(name or ""):lower()
    return n:find("bat") or n:find("slap") or n:find("medusa") or n:find("head") or n:find("stone")
end

function ASTA.isCarryingBrainrot(char)
    char = char or LP.Character
    if not char then return false end
    for _, name in ipairs(ASTA.carryFlags) do
    ...
    for _, child in ipairs(char:GetChildren()) do
        if child:IsA("Model") and child:FindFirstChildWhichIsA("BasePart", true) then
            ...
            if ASTA.akIsCarryName(child.Name) then return true end
        elseif child:IsA("Tool") and not ASTA.akIsIgnoredTool(child.Name) then
            return true
        end
    end
    return false
...
function ASTA.antiKickBlocked()
    return ASTA.antiKick and brainrotDetected
end

function ASTA.antiKickTryStart()
    if ASTA.antiKickBlocked() then
        warn("Anti-Kick: acao de velocidade bloqueada com pet equipado!")
        return false
    end
    return true
end
...
function ASTA.enableAntiKick()
    ...
    task.spawn(function()
        while ASTA.antiKick do
            task.wait(0.1) -- checagem rapida para reagir instantaneamente
            local char = LP.Character
            if char then
                brainrotDetected = ASTA.isCarryingBrainrot(char)

                if brainrotDetected then
                    -- ===== FORCA DESATIVAR TODA E QUALQUER VELOCIDADE EXTRA =====
                    ...
                    -- Desativa paths automaticos tambem
                    if autoLeftEnabled then autoLeftEnabled=false;stopAutoLeft();if autoLeftSetVisual then autoLeftSetVisual(false) end;if mobBtnRefs.autoLeft then mobBtnRefs.autoLeft(false) end end
                    if autoRightEnabled then autoRightEnabled=false;stopAutoRight();if autoRightSetVisual then autoRightSetVisual(false) end;if mobBtnRefs.autoRight then mobBtnRefs.autoRight(false) end end
                end
            end
...
toggleCarryMode=function()
    if ASTA.antiKickBlocked() then 
        print("âš ï¸ Anti-Kick: Carry Speed bloqueado com pet equipado!")
        return 
    end
...
toggleLaggerMode=function()
    if ASTA.antiKickBlocked() then 
        print("âš ï¸ Anti-Kick: Lagger Mode bloqueado com pet equipado!")
        return 
    end
...
local function speedToggleAction() 
    if ASTA.antiKickBlocked() then 
        print("âš ï¸ Anti-Kick: AÃ§Ã£o de velocidade bloqueada (tecla Q)!")
        return 
    end
    toggleCarryMode() 
    saveConfig() 
